...
class OrderStats {
    ...
    static Map<String, Long> cardsCountForCustomer(final Stream<Customer> customers) {
        Map<String, Long> answer = customers.collect(Collectors.toMap(Customer::getEmail, customer -> customer.getOrders().stream()
    ...
    static Optional<String> mostPopularCountry(final Stream<Customer> customers) {
        Optional<String> answer = customers.collect(Collectors.groupingBy(customer -> customer.getAddress().getCountry(), Collectors.counting()))
